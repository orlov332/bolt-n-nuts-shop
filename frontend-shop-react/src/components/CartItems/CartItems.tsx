...
export default function CartItems({ items, isEditable }: CartItemsProps) {
  ...
  const totalPrice: number = items.reduce((total, item) => (item.count * (item.product.price || 0) + total), 0);
      ...
      <List disablePadding>
        {items.map((cartItem: CartItem) => (
          <ListItem className={classes.listItem} key={cartItem.product.id}>
            ...
            <Typography
              variant="body2">{formatAsPrice((cartItem.product.price || 0))} x {cartItem.count} = {formatAsPrice((cartItem.product.price || 0) * cartItem.count)}</Typography>
          </ListItem>
        ))}
